...
*typeof() shows the stored type whilst class() refers to the object's class. So a value can be stored as a double whilst being numeric. Also the : method makes them integers.*
...
*a) it treats them all like doubles. b) the L makes the value an integer*
...
*Paste combines a list of strings with gaps between them. Cat prints straight to the console and removes the quotation marks and glue allows you to print multiple words within the same quotation marks.*
...
*You can't assign a variable as a number name. = is another way of assigning like <-, == is used as an equality measure.*
...
*Factors store the worded data as integer labels.*
...
*Fills by columns automatically and the second matrix creation specifies it is filled by row. They end up being transposes of each other.*
...
*R will recycle/repeat values if not enough are provided. A 3D array is like a stack of multiple 2D arrays.*
...
*Vectors can only contain one data type, lists can contain multiple. Named lists make them easier to access*
...
*Tribble is used when you want to fill by rows instead of columns. Data frames are different to tibbles as tibbles is quicker to run marginally and also when you select columns it keeps them as a tibble instead of converting it to a vector.*
...
*Glimpse shows a few more data records for each column.*
...
*Another csv file was created. Opening it again will ensure you didn't make a mistake.*
...
# Problem code:
filter(babynames, name = "Sea") # was '=' not '=='
filter(babynames, name == Sea) # didn't have Sea in quotes
```
...
The highest number is 99686 which was Linda in 1947. The largest proportion was 0.0815 (3 s.f.) which was John in 1880. They are not the same which makes sense when you consider populations would have been lower in 1880 so a higher proportion would still mean less people than a lower proportion of a higher population.
...
*I think table 1 is tidy as each row is its own observation and each column has a unique variable. Table 2 has the same observations over multiple rows. Tables 4a and 4b don't include all relevant data. Table 3 has rate which is a combination of two variables in thee same column.*
...
chol_long <- cholesterol_data |>
  pivot_longer(
    cols = c(`oat_bran_free`, `oat_bran`),    # Columns to pivot
    names_to = "Type",           # Name for the key column
    values_to = "Number of Cholesterol"          # Name for the value column
  )
...
chol_long |>
 group_by(Type) |>
  mutate(
   count = n(),
   mean = mean('Number of Cholesterol'),
   sd = sd('Number of Cholesterol')
  ) |>
  ungroup()
chol_long
```
